...
(function() {
  ...
  // Default tracking server - should be configured in manifest or storage
  const DEFAULT_TRACKING_SERVER = 'https://your-server.com';
  let trackingServer = DEFAULT_TRACKING_SERVER;
  ...
  function createTrackingPixel(uuid) {
    ...
    img.src = `${trackingServer}/img/${uuid}`;
    img.width = 1;
    img.height = 1;
  ...
  function loadSettings() {
    chrome.storage.sync.get(['trackingEnabled', 'trackingServer'], (result) => {
      trackingEnabled = result.trackingEnabled !== false;
      trackingServer = result.trackingServer || DEFAULT_TRACKING_SERVER;
      console.log('[ReadReceipt] Tracking enabled:', trackingEnabled);
      console.log('[ReadReceipt] Tracking server:', trackingServer);
    });
  }
