/**
 * Background script for Read Receipt extension (Firefox)
 * Handles tracking pixel blocking when viewing sent folder
 */

console.log('[ReadReceipt] Background script started (Firefox).');

// Store blocked requests in memory
let blockedRequests = [];
    ...
    (details) => {
      ...
      chrome.tabs.get(details.tabId, (tab) => {
        if (chrome.runtime.lastError) {
          console.error('[ReadReceipt] Error getting tab:', chrome.runtime.lastError);
          return;
        }
      ...
      if (tab && tab.url && isSentFolder(tab.url)) {
        return { cancel: true };
      }
